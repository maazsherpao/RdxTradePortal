...
$getCusCurrency = $db_con->prepare("SELECT currency_id FROM customers WHERE id = :id");
$getCusCurrency->execute(array(":id"=>$userID));
$ciVal = $getCusCurrency->fetch(PDO::FETCH_ASSOC);
$getCurrency = $db_con->prepare("SELECT * FROM customer_currency WHERE id = :id");
$getCurrency->execute(array(":id"=>$ciVal['currency_id']));
...
$symbol = strtoupper($getResult['currency']);
...
$query="SELECT p.id, p.title, p.price,p.trade_price,p.image, ci.id,ci.product_sizes_id,ci.quantity, ci.quantity * p.price AS subtotal
            FROM cart_items ci
...
if($num>0){
    ...
    while( $row = $stmt->fetch(PDO::FETCH_ASSOC)){
        extract($row);
        $newPrice = $trade_price*$valueC*$quantity;
        $conPrice = $trade_price*$valueC;
        ...
        echo "<td>";
            ...
            echo "<td>{$symbol} " . number_format($conPrice, 2, '.', ',') . "</td>";
            echo "<td>";

                        echo "<div class='input-group'>";
                        if(isset($_GET['quantity'])){
                            echo "<input type='number' name='quantity' min='1' value='{$quantityProduct}' class='form-control'>";
                          }else {
                            echo "<input type='number' name='quantity' min='1' value='{$quantity}' class='form-control'>";
                           }
    ...
    echo "<td>{$symbol} " . number_format($total, 2, '.', ',') . "</td>";
    echo "<td>";
